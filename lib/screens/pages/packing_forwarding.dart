...
class PackingForwarding extends StatefulWidget {
  const PackingForwarding({super.key});

  @override
  State<PackingForwarding> createState() => _PackingForwardingState();
}

class _PackingForwardingState extends State<PackingForwarding> {
  ...
  Future<dynamic> fetchData() async {
    try {
      ...
      final uri = Uri.parse(
          "http://10.177.48.186:8080/DesignSpace/API/PackingForwarding");
          ...
          try {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                String readyText =
                    "${data[index]["JobNo"]}-${data[index]["Particular"]}   "
                    "${data[index]["PartyName"]}@${data[index]["prepared_by"]}#"
                    "${data[index]["JobCategory"]}#${data[index]["JobTitle"]}";
