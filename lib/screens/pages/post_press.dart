...
class PostPress extends StatefulWidget {
  const PostPress({super.key});

  @override
  State<PostPress> createState() => _PostPressState();
}
...
class _PostPressState extends State<PostPress> {
  ...
  Future<dynamic> fetchData() async {
    try {
      ...
      final uri = Uri.parse(
          "http://10.177.48.186:8080/DesignSpace/API/PostPress");
          ...
          try {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                String readyText =
                    "${data[index]["JobNo"]}-${data[index]["Particular"]}-${data[index]["Serial"]}   "
                    "${data[index]["PartyName"]}@${data[index]["prepared_by"]}#"
                    "${data[index]["printing_type"]}";
